...
const AlertPage = () => {
  ...
  const getCameras = (async () => {
    try {
      ...
      if (response.ok) {
        ...
        const mappedCameras = await camerasList.allCamera.map((cam) => ({
          userId: cam.userId,
  ...
  const getAlerts = (async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/alert/getAllAlerts`, {
        method: 'GET',
        headers: {
  ...
  const AlertPageWrapper = styled.div`
  body {
    display: flex;
  ...
  .button {
    ...
    background: linear-gradient(to right, #004651, #2CB02A);
  }
  
  .button:hover {
    /* Change the gradient direction on hover */
    background: linear-gradient(to bottom, #2CB02A, #004651);
  }
  
  .button:active {
    /* Add a box shadow on active */
    ...
    <div>
      ...
      <div className={classes.cardContainer}>
        ...
        <Card className={classes.card} variant="outlined">
          ...
          <Card className={classes.card} variant="outlined">
            <CardContent className={classes.cardContent}>
              <Typography variant="h5">Send Custom Alert</Typography>
            </CardContent>

                <DialogContent>
                  {/* Render the SMSForm component within the modal */}
                  <SMSForm />
                </DialogContent>
                <DialogActions>
