...
const AlertPage = () => {
  ...
  const handleView = () => {
    if (selectedAlert) {
      ...
      setCAlerts((cAlerts) => ({
        ...cAlerts,
        [selectedCamera]: cAlerts[selectedCamera].filter(
  ...
  const getAlerts = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/alert/alertById`,
        {
          method: "GET",
          headers: {
      ...
      if (response.ok) {
        ...
        const formattedAlerts = await data.alerts?.map((alert) => {
          ...
          const formattedTime = dateObject
            ...
            .split("T")[1]
            .split(".")[0]; // "hh:mm:ss"
          const camName = cameras?.find(
            (cam) => cam.camLid === alert.camLid
          )?.camName;
          const camLat = cameras?.find(
            (cam) => cam.camLid === alert.camLid
          )?.camLat;
          const camLon = cameras?.find(
            (cam) => cam.camLid === alert.camLid
          )?.camLon;
  ...
  const updateRead = async (msgId, read_status) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/alert/readStatus`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
        ...
        <DialogContent>
          {selectedAlert && (
            <div>
              ...
              <Typography>
                {selectedAlert.camLat},{selectedAlert.camLon}
              </Typography>
            </div>
          )}
        </DialogContent>
        <DialogActions>
          ...
          {!viewedAlert[selectedCamera].includes(selectedAlert) && (
            <>
              ...
              <Button
                onClick={() =>
                  updateRead(selectedAlert.msgId, selectedAlert.read)
                }
              >
                Mark As Viewed
              </Button>
